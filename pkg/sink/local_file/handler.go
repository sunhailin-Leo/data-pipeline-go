...
func (l *LocalFileSinkHandler) InitSink() {
	switch l.sinkLocalFileCfg.FileFormatType {
	case utils.LocalFileCSVFormatType:
		file, err := os.OpenFile(l.sinkLocalFileCfg.FileName+".csv", os.O_WRONLY|os.O_CREATE|os.O_APPEND, 0o600)
		if err != nil {
			logger.Logger.Error(utils.LogServiceName +
	...
	case utils.LocalFileTextFormatType:
		file, err := os.OpenFile(l.sinkLocalFileCfg.FileName+".txt", os.O_WRONLY|os.O_CREATE|os.O_APPEND, 0o600)
		if err != nil {
			logger.Logger.Error(utils.LogServiceName +
