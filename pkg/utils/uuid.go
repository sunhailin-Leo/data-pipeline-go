...
// Copy from go-uuid - github.com/cmackenzie1/go-uuid
import (
	"crypto/rand"
	"encoding/binary"
	"encoding/hex"
...
// StringWithoutDash returns the UUID in "hex-no-dash" string format.
//
//	56c450b3255d4a2aa761cd1b1bf028e2
func (uuid UUID) StringWithoutDash() string {
	var buf [32]byte
	encodeHexWithoutDash(buf[:], uuid)
	return string(buf[:])
}
...
func GetUUID() string {
	generateUUID, err := NewV4()
	if err != nil {
		return ""
	}
	return generateUUID.StringWithoutDash()
}
